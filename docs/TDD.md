...
The structured plan returns one to three ranked hypotheses and exactly one material question. Supported semantic pairs are `post_outcome → feature_availability`, `target_proxy → target_proxy_association`, and `metric_mismatch → metric_contract`; split, group, and preprocessing checks run automatically in deterministic code. `target_proxy` takes precedence when provenance says a value is constructed from the target; `post_outcome` covers a post-decision measurement that is not itself target-derived. Hypotheses may name only reconstructed model features and must include assumptions, source references, concise rationale, and a falsifier. Application code owns dispatch, evidence, correction, finding transitions, reports, and the canonical answer choices for the selected mechanism. Model-authored question text is retained, but code normalizes it to a single-choice contract (`before/after/depends/unknown` for availability and `independent/derived_from_target/assigned_after_outcome/unknown` for provenance) so downstream evidence never depends on free-form option wording. Unsupported output degrades visibly instead of being reinterpreted. Provider transport, rate-limit, refusal, structured-behavior, and unexpected-runtime failures are classified separately without persisting raw exception text. The agent cannot write metric values directly. Structured output is validated and bounded by a five-turn limit.
