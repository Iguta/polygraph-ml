...
Before enabling merges, add the repository Actions secret `OPENAI_API_KEY` and configure `gpt-5.6-terra` as a required status check in the `dev` branch protection rule. The reviewer covers up to four bounded textual chunks (40 files / 150,000 patch characters each), refuses approval if GitHub omits an unexpected patch, and posts one updatable PR comment. Textual generated contracts, client code, and evidence remain fully reviewable. Only the exact paths for the large non-executable UCI table and binary `.skops` artifact are inventoried instead of inlined; their hashes, generator/derivation, safe-type inspection, manifest, and tests remain in the review. It is a merge gate, not a substitute for human review.
