...
Before enabling merges, add the repository Actions secret `OPENAI_API_KEY` and configure `gpt-5.6-terra` as a required status check in the `dev` branch protection rule. The reviewer covers up to four bounded textual chunks (40 files / 150,000 patch characters each), refuses approval if GitHub omits an unexpected patch, and posts one updatable PR comment. Textual generated contracts, client code, evidence, and any textual artifact patch GitHub supplies remain fully reviewable. Only a genuinely omitted patch at the exact large UCI table or binary `.skops` path uses the inventory path: the same PR must expose the exact digest in a textual root-manifest patch; the reviewer fetches both immutable Git blobs, strips only ASCII base64 whitespace, decodes strictly, enforces byte limits, scans for credentials, and requires the artifact SHA-256 to match the reviewed manifest before proceeding. Their generator/derivation, safe-type inspection, and tests also remain in the review. It is a merge gate, not a substitute for human review.
